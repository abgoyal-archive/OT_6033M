...
//porting P398_JRD458275_For_JRDSH77_CU_JB_ALPS.JB.MP.V1_P398_.tar.gz begin
/*
Check segment sequence number for validity.
 *
 * Segment controls are considered valid, if the segment
 * fits to the buffersize, which means seq <rcv_nxt+rcv_buffersize
 *
 Just for some case: server always transmits FIN(seq number is huge larger than expected one)
 */
   
static int tcp_sequence_buffersize(const struct tcp_sock *tp, struct sock *sk, u32 seq, u32 end_seq)
{
      return !before(end_seq, tp->rcv_wup) &&
              !after(seq, tp->rcv_nxt + sk->sk_rcvbuf);
}
//porting P398_JRD458275_For_JRDSH77_CU_JB_ALPS.JB.MP.V1_P398_.tar.gz end
			      ...
			      const struct tcphdr *th, int syn_inerr)
{
	...
	if (!tcp_sequence(tp, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq)) {
		 ...
		 */
	    //porting P398_JRD458275_For_JRDSH77_CU_JB_ALPS.JB.MP.V1_P398_.tar.gz begin
     if(th->fin&&(!tcp_sequence_buffersize(tp, sk,TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq)))
      {
        printk(KERN_ERR "ADDLOG tcp_validate_incoming goto discard");
        tcp_reset(sk);
        goto discard;
      }
     //porting P398_JRD458275_For_JRDSH77_CU_JB_ALPS.JB.MP.V1_P398_.tar.gz end
