...
static int8_t fan5405_disable_charging(struct fan5405_struct *fan_struct, uint8_t disable){
        ...
        mt_set_gpio_mode(GPIO209, 0);   /*mode 0 as gpio*/
        mt_set_gpio_dir(GPIO209, GPIO_DIR_OUT);         /*griffin:(DPIDE)GPIO30; martell:GPIO202 FIXME*/
        mt_set_gpio_out(GPIO209, (disable == 0)? GPIO_OUT_ZERO : GPIO_OUT_ONE); /*disable == 0: charging controlled by i2c line FIXME*/
#else
