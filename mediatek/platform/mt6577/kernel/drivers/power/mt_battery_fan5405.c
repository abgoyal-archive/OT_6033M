...
static int pre_charger_protect_status;
...
#define CHG_MIN_BYD_BATT_ID  1000
#define CHG_MAX_BYD_BATT_ID  1300
#define CHG_MIN_SCUD_BATT_ID  2100
#define CHG_MAX_SCUD_BATT_ID  2350
#define CHG_MIN_LISHEN_BATT_ID  1600
#define CHG_MAX_LISHEN_BATT_ID  1800
#define CHG_MIN_LISHEN_BATT_ID_DIABLOHD 1300
#define CHG_MAX_LISHEN_BATT_ID_DIABLOHD 1500
#define CHG_MIN_SCUD_BATT_ID_DIABLOHD 1900
#define CHG_MAX_SCUD_BATT_ID_DIABLOHD 2100
int battery_id_check;
...
int g_bat_temperature_above45=0;
int g_bat_temperature_above38=0;
...
#ifdef DIABLO_BAT_ID_CHECK
	if( ((bat_id_vol >= CHG_MIN_BYD_BATT_ID) && (bat_id_vol <= CHG_MAX_BYD_BATT_ID)) || ((bat_id_vol >= CHG_MIN_SCUD_BATT_ID) && (bat_id_vol <= CHG_MAX_SCUD_BATT_ID)) || ((bat_id_vol >= CHG_MIN_LISHEN_BATT_ID) && (bat_id_vol <= CHG_MAX_LISHEN_BATT_ID)) )
	{
	    if((bat_id_vol >= CHG_MIN_SCUD_BATT_ID) && (bat_id_vol <= CHG_MAX_SCUD_BATT_ID))
	    	{
               battery_id_check = 1;
		}
	    else if((bat_id_vol >= CHG_MIN_LISHEN_BATT_ID) && (bat_id_vol <= CHG_MAX_LISHEN_BATT_ID))
	    	{
               battery_id_check = 2;
		}
	    else
		...
		printk("DIABLO_BAT_ID_CHECK pass\n");
	}
	else       
#endif

#ifdef DIABLOHD_BAT_ID_CHECK
        if( ((bat_id_vol >= CHG_MIN_SCUD_BATT_ID_DIABLOHD) && (bat_id_vol <= CHG_MAX_SCUD_BATT_ID_DIABLOHD)) || ((bat_id_vol >= CHG_MIN_LISHEN_BATT_ID_DIABLOHD) && (bat_id_vol <= CHG_MAX_LISHEN_BATT_ID_DIABLOHD)) )
        {
            if((bat_id_vol >= CHG_MIN_SCUD_BATT_ID_DIABLOHD) && (bat_id_vol <= CHG_MAX_SCUD_BATT_ID_DIABLOHD))
                {
                battery_id_check = 1;
		  }
	     else
	     	 {
                ...
                printk("DIABLOHD_BAT_ID_CHECK pass\n");
        }
        else
#endif
...
int get_pmic_flag=0;
#ifdef TELSA_FAN5405_GPIO
int gpio_number   = GPIO209;		/*Diablo disable pin ??? FIXME*/
#else
...
if (BMT_status.bat_charging_state == CHR_ERROR ) {
    ...
    else
    {
		...
		#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
	   		#ifdef DIABLO_FUEL_GAUGE
			if(BMT_status.temperature >= 45)
			{
			fan_struct->ops->set_con2_val(fan_struct, 0x1E, 0x01, 0x00);
			}
			else
			{
			fan_struct->ops->set_con2_val(fan_struct, 0x2a, 0x01, 0x00);
			}
	   		#else 
	   		fan_struct->ops->set_con2_val(fan_struct, 0x2a, 0x01, 0x00);       /*set Vout = 4.20V, otg pin active high, disable otg pin*/
	   		#endif
	   	#else
		#ifdef TELSA_FUEL_GAUGE
            ...
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:fan5405] charger enable !\r\n");
                      ...
                      //  select_charging_curret();
...
#ifdef DIABLOHD_CHARGING_POLICY_SUPPORT
                        //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                        /* Reset charging current when temperature beyond rank limition - Troy 2012/11/22 */
                        int i;
                        int temperature_threshold[2] = { 38, 45 };
                        int charging_Iinlim[3] = { 0x03, 0x01, 0x00 };
                        int charging_Iocharge[3] = { 0x07, 0x07, 0x00 };
                
                        for ( i = 0; i < 2; i ++ )
                        {
                                if ( BMT_status.temperature < temperature_threshold[i] )
                                        break;
                        }
                                       
                        if ( Iinlim > charging_Iinlim[i] )
                        {
                                Iinlim = charging_Iinlim[i];
                                              
                                Iocharge = charging_Iocharge[i];
                     }
#endif
#if defined(TELSA_FUEL_GAUGE)||(DIABLO_FUEL_GAUGE)
                        int i;
                        int temperature_threshold[2] = { CHARGE_TEMPERATURE_STEP1, CHARGE_TEMPERATURE_STEP2 };
                        int charging_Iinlim[3] = { 0x03, 0x03, 0x03 };
                        int charging_Iocharge[3] = { 0x07, 0x02, 0x02 };
                        if((BMT_status.charger_type == CHARGING_HOST)||(BMT_status.charger_type == STANDARD_CHARGER))
                        {
                            for ( i = 0; i < 2; i ++ )
                            {
                                   if ( BMT_status.temperature < temperature_threshold[i] )
                        ...
                        {
			        uint8_t v4,v5;
				fan_struct->ops->set_con1_value(fan_struct, Iinlim, 0x00, 1, 0, 0, 0);
			        fan_struct->ops->set_con4_val(fan_struct, 0, 0x02, 2);
				fan_struct->ops->set_con5_val(fan_struct, 0, 0x00, 0x04);
				...
				{
					printk("---------------------------------------------\n");
			        	v4 = fan_struct->ops->get_reg_value(fan_struct, 0x04);
			        	v5 = fan_struct->ops->get_reg_value(fan_struct, 0x05);
			        	printk("---------------------------------------------\n");
			        	printk("fan5405_reg4.. = %x\n",v4);
...
#ifdef DIABLOHD_CHARGING_POLICY_SUPPORT
	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        /* Check temperature whether switch rank threshold - Troy 2012/11/22 */
        if ( g_bat_temperature_pre != 0 && upmu_is_chr_det() == KAL_TRUE )
       	{
                int chr_res = 0;

                if ( BMT_status.temperature < 38 && g_bat_temperature_pre < 38 || BMT_status.temperature >= 38 && BMT_status.temperature < 45 && g_bat_temperature_pre >= 38 && g_bat_temperature_pre < 45 || BMT_status.temperature >= 45 && g_bat_temperature_pre >= 45 )
      	                chr_res = 1;

                if ( chr_res == 1 )
        	{
			pchr_turn_on_charging_fan5405();
                        printk( KERN_INFO, "[Troy][Charging]: temperature rank changed! \n");
                }
       	}
...
#if defined(DIABLO_FUEL_GAUGE)||defined(TELSA_FUEL_GAUGE)
	  ...
	  if ( g_bat_temperature_pre != 0 && upmu_is_chr_det() == KAL_TRUE )
         {
       	if((BMT_status.charger_type == CHARGING_HOST)||(BMT_status.charger_type == STANDARD_CHARGER))
	  	   ...
	  	   {
                 g_bat_temperature_above45 =1;				 
	          }
                if(g_bat_temperature_above45 == 1)
                {
                    if(BMT_status.temperature < (CHARGE_TEMPERATURE_STEP2-2))
                	{
                    g_bat_temperature_above45 =0;
			pchr_turn_on_charging_fan5405();
			}
		   }
		   if(BMT_status.temperature >= CHARGE_TEMPERATURE_STEP1)
	  	   {
                 g_bat_temperature_above38 =1;
		    //diablo change this temperature from 38 to 42
	          }
                if(g_bat_temperature_above38 == 1)
                {
                    if(BMT_status.temperature < (CHARGE_TEMPERATURE_STEP1-2))
                	{
                    g_bat_temperature_above38 =0;
			pchr_turn_on_charging_fan5405();
			}
		   }
                if ( (BMT_status.temperature >= CHARGE_TEMPERATURE_STEP1 && g_bat_temperature_pre < CHARGE_TEMPERATURE_STEP1) || (BMT_status.temperature >= CHARGE_TEMPERATURE_STEP2 && BMT_status.temperature < CHARGE_TEMPERATURE_STEP2 ))
        	   {
			pchr_turn_on_charging_fan5405();
                        printk( KERN_INFO, "[Troy][Charging]: temperature rank changed! \n");
                }
...
/*
  ...
  */
  //this status is for our own control ,fan_struct->chg_fault_status is for fan5405 HW,so we need to add our own defined flage
	 BMT_status.bat_charging_state = CHR_ERROR;
	...
	BMT_status.charger_protect_status = BATTERY_LOW_TEMP;
       pre_charger_protect_status = BMT_status.charger_protect_status;
#endif
        return PMU_STATUS_FAIL;       
    }
    #endif            
...
#ifdef FEATURE_RECOVERY_FROM_HIGH_TEMPERATURE
    if((pre_charger_protect_status == BATTERY_OVER_TEMP) || (pre_charger_protect_status == BATTERY_LOW_TEMP))
    {
        if((BMT_status. temperature>=BATTERY_LOW_RECOVERY_CHARGING_TEMP) && (BMT_status. temperature<= BATTERY_OVER_RECOVERY_CHARGING_TEMP))
        {
	      ...
	      printk("recovery from high temperature BMT_status.charger_protect_status = 0!%d\n",BMT_status. temperature);
        }
    }
    if(((pre_charger_protect_status == BATTERY_OVER_TEMP) || (pre_charger_protect_status == BATTERY_LOW_TEMP)) && (BMT_status.charger_protect_status == 0))
    {
        pre_charger_protect_status = 0;
        BMT_status.bat_charging_state = CHR_PRE;
        ...
        {            
            ...
            return;
                ...
                if( (((fan5405_status & 0x30) == 0x20) && (fan_struct->config_count == 0) && (fan_struct->chg_fault_status == CHARGE_NORMAL_STATUS)) ||
                        ...
                        {
                                printk(KERN_INFO "jrd %s: Fan5405  fault %d,%d \n", __func__,(fan5405_status & 0x30),fan_struct->config_count);
