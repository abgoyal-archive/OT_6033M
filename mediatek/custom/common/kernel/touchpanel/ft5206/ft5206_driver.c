...
#ifdef FW_UPGRADE_ENABLE
static unsigned char CTPM_TURLY_OGS_FW[]=
{
#include "Ft5x16_Lib_Diablo_Ver0x19_ID0x5a_app.h"
};

static unsigned char CTPM_JINLAI_FW[]=
{
	#include "Ft5x16_Lib_Diablo_LaiBao_V0x91_ID0x55_app.h"
};

static u8 fts_ctpm_get_upg_ver(void)
{
	unsigned int ui_sz = 0;
	if ((tp_factory_id_cmd == TS_TYPE_TRULY) || (tp_factory_id == TS_TYPE_TRULY)) {
		ui_sz = sizeof(CTPM_TURLY_OGS_FW);
		printk("fts_ctpm_get_upg_ver ui_sz,value1 value2 is %4x %2x %2x xxxxxxxxxx\n", \
		ui_sz,CTPM_TURLY_OGS_FW[ui_sz - 2],CTPM_TURLY_OGS_FW[ui_sz - 1]);
		if (ui_sz > 2) {    
			return CTPM_TURLY_OGS_FW[ui_sz - 2];
		} else {    
			/*TBD, error handling?*/
			return 0xff; /*default value*/
		}
	} else if ((tp_factory_id_cmd == TS_TYPE_JINLAI) || (tp_factory_id == TS_TYPE_JINLAI)){
		ui_sz = sizeof(CTPM_JINLAI_FW);
		printk("fts_ctpm_get_upg_ver ui_sz,value1 value2 is %4x %2x %2x xxxxxxxxxx\n", \
		ui_sz,CTPM_JINLAI_FW[ui_sz - 2],CTPM_JINLAI_FW[ui_sz - 1]);
		if (ui_sz > 2) {    
			return CTPM_JINLAI_FW[ui_sz - 2];
		} else {    
			/*TBD, error handling?*/
			return 0xff; /*default value*/
		}
...
static int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	/*=========FW upgrade========================*/
	if ((tp_factory_id_cmd == TS_TYPE_TRULY) || (tp_factory_id == TS_TYPE_TRULY)) {
		pbt_buf = CTPM_TURLY_OGS_FW;
		/*call the upgrade function*/
		i_ret =  fts_ctpm_fw_upgrade(pbt_buf,sizeof(CTPM_TURLY_OGS_FW));
		if (i_ret != 0)	{
		...
		pbt_buf = CTPM_JINLAI_FW;
		/*call the upgrade function*/
		i_ret =  fts_ctpm_fw_upgrade(pbt_buf,sizeof(CTPM_JINLAI_FW));
		if (i_ret != 0)	{
			printk("[FTS] upgrade failed i_ret = %d.\n", i_ret);
		}
 		else {
...
static void tp_debug(int tp_debug_value)
{
	u8 tp_factory_id_cmd1 = TS_TYPE_UNKNOWN;
	u8 tp_factory_id1 = TS_TYPE_UNKNOWN;
	
	if (tp_debug_value == 1) {
	read_reg(0xa8, &tp_factory_id_cmd1);
	printk("tp_factory_id_cmd = %x, read ID !\n", tp_factory_id_cmd1);
	
	tp_factory_id1 = fts_ctpm_get_panel_factory_setting();
	printk("tp_factory_id = %x, read ID !\n", tp_factory_id1);
	}	
...
//wake
	...
	if((i2c_smbus_write_i2c_block_data(i2c_client, 0xb6, 1, &palmarea_level))< 0)
	    {
		   TPD_DMESG("I2C read palm level error, line: %d\n", __LINE__);
	    }
	...
	if ((tp_factory_id_cmd != TS_TYPE_TRULY) && (tp_factory_id_cmd != TS_TYPE_MUTTO) && (tp_factory_id_cmd != TS_TYPE_JINLAI))
	{
		tp_factory_id = fts_ctpm_get_panel_factory_setting();
		printk("tp_factory_id = %x, read ID !\n", tp_factory_id);
	}
