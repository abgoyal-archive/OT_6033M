...
MSDK_SENSOR_CONFIG_STRUCT OV8850SensorConfigData;

kal_uint32 OV8850_FAC_SENSOR_REG;

MSDK_SCENARIO_ID_ENUM OV8850CurrentScenarioId = ACDK_SCENARIO_ID_CAMERA_PREVIEW;

/* FIXME: old factors and DIDNOT use now. s*/
SENSOR_REG_STRUCT OV8850SensorCCT[]=CAMERA_SENSOR_CCT_DEFAULT_VALUE;
SENSOR_REG_STRUCT OV8850SensorReg[ENGINEER_END]=CAMERA_SENSOR_REG_DEFAULT_VALUE;
...
unsigned short awb_ratio_low2bit = 0;
...
signed char otp_read_wb_group(signed char index)
{
	...
	otp_read(otp_addr+6, &rg_ratio);
	otp_read(otp_addr+7, &bg_ratio);
		
	otp_read(otp_addr+10, &awb_ratio_low2bit);//read the low 2bit for R/G,B/G
	
	rg_ratio = (rg_ratio<<2)+(awb_ratio_low2bit>>6);
	bg_ratio = (bg_ratio<<2)+((awb_ratio_low2bit&0x30)>>4);
...
bool otp_apply_wb(unsigned short golden_rg, unsigned short golden_bg)
{
	...
	unsigned short reg3406, reg5000,reg5001;
	...
	OV8850DB("golden sample: golden_rg=%d, golden_bg=%d\n", golden_rg, golden_bg);
	OV8850DB("this sample: rg_ratio=%d, bg_ratio=%d\n", rg_ratio, bg_ratio);
	OV8850DB("cmp_rg=%d, cmp_bg=%d\n", cmp_rg, cmp_bg);
	OV8850DB("ratio_r=%d, ratio_g=%d, ratio_b=%d\n", ratio_r, ratio_g, ratio_b);
	OV8850DB("gain_r=0x%x, gain_g=0x%x, gain_b=0x%x\n", gain_r, gain_g, gain_b);
	reg3406 = OV8850_read_cmos_sensor(0x3406);
	reg5000 = OV8850_read_cmos_sensor(0x5000);
	reg5001 = OV8850_read_cmos_sensor(0x5001);
	OV8850DB("reg3406=0x%x, reg5000=0x%x, reg5001=0x%x\n", reg3406, reg5000, reg5001);
	return 1;
}
...
void OV8850_write_shutter(kal_uint32 shutter)
{
...
void write_OV8850_gain(kal_uint16 gain)
{
...
*
*************************************************************************/
void OV8850_SetGain(UINT16 iGain)
{
	UINT16 gain_reg=0;
	unsigned long flags;
	spin_lock_irqsave(&ov8850mipiraw_drv_lock,flags);
...
*
*************************************************************************/
kal_uint16 read_OV8850_gain(void)
{
    kal_uint8  temp_reg;
...
void OV8850_camera_para_to_sensor(void)
{
    kal_uint32    i;
...
*
*************************************************************************/
void OV8850_sensor_to_camera_para(void)
{
...
*
*************************************************************************/
kal_int32  OV8850_get_sensor_group_count(void)
{
    return GROUP_TOTAL_NUMS;
}

void OV8850_get_sensor_group_info(kal_uint16 group_idx, kal_int8* group_name_ptr, kal_int32* item_count_ptr)
{
   switch (group_idx)
   {
        case PRE_GAIN:
...
void OV8850_get_sensor_item_info(kal_uint16 group_idx,kal_uint16 item_idx, MSDK_SENSOR_ITEM_INFO_STRUCT* info_ptr)
{
...
kal_bool OV8850_set_sensor_item_info(kal_uint16 group_idx, kal_uint16 item_idx, kal_int32 ItemValue)
{
//   kal_int16 temp_reg;
   kal_uint16  temp_gain=0,temp_addr=0, temp_para=0;
...
void OV8850PreviewSetting(void)
{	
...
void OV8850CaptureSetting(void)
{	
...
UINT32 OV8850Open(void)
{
	...
	OV8850DB("OV8850Open enter :\n ");
	OV8850_write_cmos_sensor(0x0103,0x01);// Reset sensor
    	mDELAY(5);
	//read CTYPE
	mt_set_gpio_mode(49, 0);	/*mode 0 as gpio*/
        mt_set_gpio_dir(49, GPIO_DIR_IN);  /*griffin:(DPIVSYNC)GPIO21; martell:GPIO200; get STAT pin status FIXME*/
        ret = mt_get_gpio_in(49);		/*FIXME*/
	printk("xxxxxxxxxxxxallen read ctype : %d xxxxxxxxxx",ret);
...
UINT32 OV8850GetSensorID(UINT32 *sensorID) 
{
	...
	OV8850DB("OV8850GetSensorID enter :\n ");
	OV8850_write_cmos_sensor(0x0103,0x01);// Reset sensor
    mDELAY(10);
   ...
   // PR 449723  rui.cui@tcl.com 2013-05-06 begin 
    {//OV8850 2B version, P8V07C, OTP read bank31 (0x3D00 = 0x02 or 0x01) && bank0 (0x3D00 = 0x50) 
      unsigned char  flag0 = 0;
      unsigned char  flag31 = 0;
      OV8850_write_cmos_sensor(0x100, 0x00);//disable output
      OV8850_write_cmos_sensor(0x103, 0x01);//reset
      OV8850_write_cmos_sensor(0x100, 0x01);//enable output
      // select bank 31
      OV8850_write_cmos_sensor(OTP_BANK_ADDR, 0xc0 | (31));
      otp_read(OTP_DATA_ADDR, &flag31);
      otp_clear();

      // select bank 0
      OV8850_write_cmos_sensor(OTP_BANK_ADDR, 0xc0 | (0));
      otp_read(OTP_DATA_ADDR, &flag0);

      otp_clear();
      OV8850DB("otp_read(0x3d00, &flag31) = 0x%02x\n", flag31); 
      OV8850DB("otp_read(0x3d00, &flag0) = 0x%02x\n", flag0);
      
      if (*sensorID == 0x8850 && flag31 != 0x40 && flag0 == 0x50)
      {
...
// PR 449723  rui.cui@tcl.com 2013-05-06 end
}
...
*
*************************************************************************/
void OV8850_SetShutter(kal_uint32 iShutter)
{
...
*
*************************************************************************/
UINT32 OV8850_read_shutter(void)
{
...
*
*************************************************************************/
void OV8850_NightMode(kal_bool bEnable)
{
}/*	OV8850_NightMode */
...
void OV8850SetFlipMirror(kal_int32 imgMirror)
{
...
UINT32 OV8850SetTestPatternMode(kal_bool bEnable)
{
