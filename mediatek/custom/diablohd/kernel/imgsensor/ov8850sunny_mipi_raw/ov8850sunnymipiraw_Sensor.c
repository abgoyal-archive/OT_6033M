...
#ifdef OV8850_DEBUG
	#define OV8850DB(fmt, arg...) printk( "[Sunny-OV8850Raw] "  fmt, ##arg)
#else
	#define OV8850DB(x,...)
#endif

#ifdef OV8850_DEBUG_SOFIA
	#define OV8850DBSOFIA(fmt, arg...) printk( "[Sunny-OV8850Raw] "  fmt, ##arg)
#else
...
static void otp_apply_lenc(void)
{
	...
	for (i=0; i<LENC_REG_SIZE; i++)
	{
		...
		OV8850DB("otp_apply_lenc[%d], 0x%x, 0x%x\n",i, LENC_START_ADDR+i, otp_lenc_data[i]);
	}
}
...
{	
	...
	//neil_jin modify for ov8850 3A version
      ...
      OV8850_write_cmos_sensor(0x3090, 0x03); //0x02
      OV8850_write_cmos_sensor(0x3091, 0x20); //0x10
      OV8850_write_cmos_sensor(0x3092, 0x01); //0x00
      ...
      OV8850_write_cmos_sensor(0x380d, 0x48); //0x18
      OV8850_write_cmos_sensor(0x380e, 0x04); //0x07
      OV8850_write_cmos_sensor(0x380f, 0xf0); //0x80
      ...
      OV8850_write_cmos_sensor(0x5045, 0x55); //for BLC issue
      OV8850_write_cmos_sensor(0x5048, 0x0f); //for BLC issue
...
{	
    ...
    OV8850_write_cmos_sensor(0x3015, 0xca);
    OV8850_write_cmos_sensor(0x3090, 0x03);//0x02
    OV8850_write_cmos_sensor(0x3091, 0x20); //0x15
    ...
    OV8850_write_cmos_sensor(0x4837, 0x0c);
    OV8850_write_cmos_sensor(0x5045, 0x55); //for BLC issue
    OV8850_write_cmos_sensor(0x5048, 0x10); //for BLC issue    
    OV8850_write_cmos_sensor(0x0100, 0x01);
...
static void OV8850_Sensor_Init(void) // 3A version
{
	...
	//neil modify for ov8850 3A version
     ...
     OV8850_write_cmos_sensor(0x4800, 0x24); //0x14
     ...
     OV8850_write_cmos_sensor(0x4007, 0x90); //for BLC issue
     OV8850_write_cmos_sensor(0x0100, 0x01);
...
static UINT32 OV8850Open(void)
{
	...
	ov8850.pvPclk =  (13867); //neil for preview banding
	spin_unlock(&ov8850mipiraw_drv_lock);
	#ifndef MT6573
	//#if defined(MT6575)||defined(MT6577)
    	switch(OV8850CurrentScenarioId)
	...
	OV8850DB("sunny OV8850Open exit :\n ");
...
static UINT32 OV8850GetSensorID(UINT32 *sensorID) 
{
    ...
    {//OV8850 3A version, P8V07K, OTP read bank31 (0x3D00 = 0x40) && bank0 (0x3D00 = 0x50), or OTP read bank0 (0x3D00 = 0x3A)
      ...
      if (((flag31 == 0x40 && flag0 == 0x50) || flag0 == 0x3A) && *sensorID == 0x8850)
      {
        *sensorID = OV8850SUNNY_SENSOR_ID;
        return ERROR_NONE;
      }
			...
			//#if defined(MT6575)||defined(MT6577)
        			...
        			//OV8850DB("OV8850FeatureControl:OV8850CurrentScenarioId:%d\n",OV8850CurrentScenarioId);
        				case MSDK_SCENARIO_ID_CAMERA_ZSD:
		         		...
		         		default:
		            		*pFeatureReturnPara32 =  138670000; //208000000;PR 456126 rui.cui@tcl.com 2013-05-28
		            		*pFeatureParaLen=4;
		            		break;
		        	}
			#else
