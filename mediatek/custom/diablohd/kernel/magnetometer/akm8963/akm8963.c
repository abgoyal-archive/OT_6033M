...
static long AKECS_GetData(char *rbuf, int size)
{
	...
	for(i=1;i<8;i++)
    {
		rbuf[i] = AK8963_REG_ST1+i;
		ret = AKI2C_RxData( rbuf+i, 1);
		if (ret < 0) {
				printk(KERN_ERR "AKM8975 akm8975_work_func: I2C failed\n");
				return -1;
		}
	...
	switch (cmd)
	{
	   ...
	   case ECS_IOCTL_RESET:
		...
		case ECS_IOCTL_READ:
			...
			if(rwbuf [1]==AK8963_FUSE_ASAX && rwbuf[0]==3){

				AKI2C_RxData(&rwbuf[1], 1);
				rwbuf [2]= AK8963_FUSE_ASAY;
				AKI2C_RxData(&rwbuf[2], 1);
				rwbuf [3]= AK8963_FUSE_ASAZ;
				AKI2C_RxData(&rwbuf[3], 1);

				if(rwbuf[1] == 0 || rwbuf[1] == 255 || rwbuf[2] == 0 || rwbuf[2] == 255 || rwbuf[3] == 0 || rwbuf[3] == 255)
				{
					printk("before:rwbuf[1]= %d, rwbuf[2]= %d, rwbuf[3]= %d", rwbuf[1], rwbuf[2], rwbuf[3]);
					rwbuf[1]=160;
					rwbuf[2]=160;
					rwbuf[3]=160;
					printk("after:rwbuf[1]= %d, rwbuf[2]= %d, rwbuf[3]= %d", rwbuf[1], rwbuf[2], rwbuf[3]);
				}
 

			}else{

			printk("register addr= 0X%x\n", rwbuf[1]);
			ret = AKI2C_RxData(&rwbuf[1], rwbuf[0]);
			if (ret < 0)
			{
				return ret;
			}
			printk("rwbuf[1]= %d, rwbuf[2]= %d, rwbuf[3]= %d\n", rwbuf[1], rwbuf[2], rwbuf[3]);
		}
