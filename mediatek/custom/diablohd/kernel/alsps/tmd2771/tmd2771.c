...
static ssize_t tmd2771_show_config(struct device_driver *ddri, char *buf)
{
	...
	res = snprintf(buf, PAGE_SIZE, "(%d %d %d %d %d %d %x %x)\n", 
		...
		TMD2771_CMM_PPCOUNT_VALUE, TMD2771_CMM_CONTROL_VALUE);     
	return res;    
}
...
static ssize_t tmd2771_store_config(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    {        
        TMD2771_CMM_PPCOUNT_VALUE=ppcount;
        TMD2771_CMM_CONTROL_VALUE=cmmctl;
	}		
	else
	{
...
static ssize_t tmd2771_show_pscali(struct device_driver *ddri, char *buf)
{
        u16 thd_cali[2]={0};
...
static ssize_t tmd2771_store_pscali(struct device_driver *ddri, const char *buf, size_t count)
{
    	int value;
        int32_t ret;
	...
	if(!g_tmd2771_ptr)
	{
		APS_ERR("tmd2771_priv   g_tmd2771_ptr is null!!\n");
		return 0;
	}
...
static ssize_t tmd2771_factory_cali(struct device_driver *ddri,char *buf)
{
	int value;
	int32_t ret;
	if(!g_tmd2771_ptr)
...
#if JOHN_CAL 
static DRIVER_ATTR(pscali,  S_IWUSR | S_IRUGO, tmd2771_show_pscali,tmd2771_store_pscali);
#endif
static DRIVER_ATTR(psfac,	S_IWUSR | S_IRUGO,tmd2771_factory_cali,NULL);
...
static int32_t tmd_get_several_ps_data(u16 ps_stat_data[])
{
	...
	databuf[0] = TMD2771_CMM_PPCOUNT;    
	databuf[1] = TMD2771_CMM_PPCOUNT_VALUE;
	ret = i2c_master_send(g_tmd2771_ptr->client, databuf, 0x02);
	if(ret <= 0)
	{
		goto EXIT_ERR;
		return TMD2771_ERR_I2C;
	...
	tmd2771_ps_calibrate(g_tmd2771_ptr->client);
#endif 

	if((ret = tmd2771_init_client(g_tmd2771_ptr->client)))
	{
		APS_ERR("tmd2771 reinitiate client after change the configure parameter failure\n");
		goto EXIT_ERR;
	}

	for(i = 0; i < TMD_PS_CAL_TIMES; i++)
	{
		if(ret = tmd2771_read_ps(g_tmd2771_ptr->client, &ps_data[i]))
		{
...
static int32_t tmd_get_ps_cali_thd(u16 ps_thd[])
{
	...
	else
	{
		...
		if(r_buf[0] == TMD_CALI_VER0 && r_buf[1] == TMD_CALI_VER1&&r_buf[6] == TMD_CALI_VER2 &&r_buf[7]==TMD_CALI_VER3)
		{
            ...
            atomic_set(&tmd2771_obj->ps_cali_done,1);
            tmd2771_obj->cali_file_exist=1;
		}
		else
		{
...
/*we can add below code under the IC_i2C_probe.init the variables value. start */
        tmd2771_obj->first_boot =1;
        atomic_set(&obj->ps_cali_done, 0);
        tmd2771_obj->cali_file_exist= 0;    
        tmd2771_obj->ps_CTK_val =0;     
