...
#if defined(DIABLO_GREEN_LIGHT)
extern LCM_DRIVER *lcm_drv;
extern void B4_set(void);
#endif

#if defined(ENABLE_SMART_DIMM)
extern int dump_lcd_gamma_value(int data);
extern int S6e8aa0_Smart_Dimmy(void);

static ssize_t show_lcd_gamma(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s", "echo 0 for re-calculate and dump all gamma\n echo 1 for dump gamma value\n");;
}

static ssize_t set_lcd_gamma(struct device *dev,  struct device_attribute *attr, const char *buffer, size_t count)
{
    int data;
    sscanf(buffer, "%d", &data);
	
	dump_lcd_gamma_value(data);
    return count;
}
DEVICE_ATTR(lcd_dump_gamma, 0664, show_lcd_gamma, set_lcd_gamma);
#endif


#if defined(ENABLE_ACL_FUNCTION)
/*jrd add for LCD ACL function,PR350502*/
extern int set_acl_function(unsigned int enable);
extern int get_acl_status(void) ;

static struct platform_device *lcd_acl_dev;

static ssize_t show_lcd_acl(struct device *dev, struct device_attribute *attr, char *buf)
{
	int index = get_acl_status();
	return sprintf(buf, "%d\n", index);
}

static ssize_t set_lcd_acl(struct device *dev,  struct device_attribute *attr, const char *buffer, size_t count)
{
    int enable;
    sscanf(buffer, "%d", &enable);
    set_acl_function(enable);
    return count;
}

/* Attach the sysfs write method */
DEVICE_ATTR(lcd_acl_function, 0664, show_lcd_acl, set_lcd_acl);


/* Attribute Descriptor */
static struct attribute *lcd_acl_attrs[] = {
  &dev_attr_lcd_acl_function.attr,
#if defined(ENABLE_SMART_DIMM) 
  &dev_attr_lcd_dump_gamma.attr,	
#endif
  NULL
};

/* Attribute group */
static struct attribute_group lcd_acl_attr_group = {
  .attrs = lcd_acl_attrs,
...
//add by xiewei for smart dimming start
#if defined(ENABLE_SMART_DIMM)
extern void open_lcd_backlight(void);
extern void close_lcd_backlight(void);
static int count =0;
//add by xiewei for smart dimming end
#endif
...
#if defined(ENABLE_SMART_DIMM)
	if(count >=0&&count <4){
		if(count ==0){//for update kernel LOGO
			count++;
		}else{	
			if (count==1){
				close_lcd_backlight();
...
//#if defined(ENABLE_SMART_DIMM)
	//S6e8aa0_Smart_Dimmy();//for amoled smart dimmy
//#endif
...
#if defined(ENABLE_ACL_FUNCTION)
/*jrd add for LCD ACL function,PR350502*/
  lcd_acl_dev = platform_device_register_simple("jrd_lcd_acl", -1, NULL, 0);

  if (IS_ERR(lcd_acl_dev)) {
    printk("acl_dev_init: error\n");
  }

  sysfs_create_group(&lcd_acl_dev->dev.kobj, &lcd_acl_attr_group);
/*end*/
#endif
...
#if defined(ENABLE_ACL_FUNCTION)
/*jrd add for LCD ACL function,PR350502*/
    sysfs_remove_group(&lcd_acl_dev->dev.kobj, &lcd_acl_attr_group);
    platform_device_unregister(lcd_acl_dev);
/*end*/
#endif
